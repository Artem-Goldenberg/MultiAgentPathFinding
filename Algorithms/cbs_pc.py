...
def find_agents_paths(task: MAPF):
    """ Conflict based search with prioritizing conflicts optimization """

    if not validate(task): return None
    ...
    while not ast.open_is_empty():  # MAIN LOOP
        ...
        # conflict = best_conflict(node, task)
...
def find_agents_path2(task: MAPF):
    ...
    node = get_start_node(task, build_mdds=True)
    if node is None: return None
    ...
    while not ast.open_is_empty():  # MAIN LOOP
        ...
        for agent_id in conflict.agent_ids: # generate new nodes for 2 agents
            ...
            if not ast.was_expanded(neighbor):
                if update_solution_for(agent_id, neighbor, task, build_mdd=True):
                    assert neighbor.cost >= node.cost
                    ast.add_to_open(neighbor)
...
def simple_best(node: Node, task: MAPF) -> Conflict:
    def get_mdd(agent_id: int):
        mdd = node.solutions[agent_id].mdd
        if mdd is None:
            update_solution_for(agent_id, node, task, build_mdd=True)
            mdd = node.solutions[agent_id].mdd
            assert mdd.size # type: ignore
        return mdd 
...
def best_conflict(node: Node, task: MAPF) -> Conflict:
    def get_mdd(agent_id: int):
        mdd = node.solutions[agent_id].mdd
        if mdd is None:
            update_solution_for(agent_id, node, task, build_mdd=True)
